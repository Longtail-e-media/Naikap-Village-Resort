...
<?php
$moduleTablename = "tbl_nearby"; // Database table name
$moduleId = 3;                // module id >>>>> tbl_modules
$moduleFoldername = "nearby";        // Image folder name

if (isset($_GET['page']) && $_GET['page'] == "nearby" && isset($_GET['mode']) && $_GET['mode'] == "list"):
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
    if (isset($_GET['id']) && !empty($_GET['id'])):
        ...
        $articlesInfo = Nearby::find_by_id($articlesId);
    ...
    ?>
    <h3>
        <?php echo (isset($_GET['id'])) ? 'Edit Nearby' : 'Add Nearby'; ?>
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);"
                    ...
                    if (!empty($articlesInfo->image)) {
                        ...
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                ...
                                $imagePath = SITE_ROOT . 'images/nearby/' . $recimg;
                            ...
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/nearby/" . $recimg)):
                                    $filesize = filesize(SITE_ROOT . "images/nearby/" . $recimg);
                                ...
                                ?>
                                            ...
                                            <img src="<?php echo IMAGE_PATH . 'nearby/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
        ...
        $(document).ready(function () {
            $('#gallery_upload').uploadify({
                ...
                'onUploadSuccess': function (file, data, response) {
                    ...
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/nearby/uploaded_image.php', {imagefile: filename}, function (msg) {
