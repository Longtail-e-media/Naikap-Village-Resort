...
if (!empty($socialRec)) {
    foreach ($socialRec as $socialRow) {
        if(!empty($socialRow->icon)){
            $resocl .= '
            <a
                href="' . $socialRow->linksrc . '"
                target="_blank"
                class="flex items-center justify-center w-8 h-8 mr-3 border rounded-full hover:bg-[#f5f5dc] hover:text-gray-800 sm:mr-4 lg:mr-3 xl:mr-4 transition-all duration-200 ease-in-out"
            >
                <i class="' . $socialRow->icon . '"></i>
            </a>';
        }
        // else{
        //     $resocl .= ' <a href="' . $socialRow->linksrc . '" target="_blank"><img src="' . IMAGE_PATH . 'social/'. $socialRow->image . '" ></i></a>';
